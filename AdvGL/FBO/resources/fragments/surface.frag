...
vec3 CalcPointLight(PointLight light, Material material, vec3 Normal, vec3 viewPos)
{
    ...
    vec3 diff = vec3(texture(material.diffuse, Coord)) * abs( dot(Normal, normalize(dist_source))); 
...
vec3 CalcSpotLight(SpotLight light, myTexture texture, vec3 Normal, vec3 viewPos, vec3 objectColor)
{
    ...
    float strength = light.strength / (dot(dist_source, dist_source) + 20.0f);
...
void main()
{
    vec3 result = CalcPointLight(light_1, material_2, Normal, viewPos);
